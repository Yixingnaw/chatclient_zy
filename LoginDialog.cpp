...
void LoginDialog::connectSlots()
{
    ...
    //界面数据参数传递
    connect(Socket::GetInstance(),&Socket::LOGIN_MSG_ACK,this,&LoginDialog::LOGIN_MSG_ACK);
}
...
void LoginDialog::onRegisterCount()
{
 ...
 //RegisterWidget注册界面类，缺乏回馈
    RegisterWidget *registerWidget = new RegisterWidget(window);
    QObject::connect(registerWidget, &RegisterWidget::registerSignal, [&](const QString &jsonstring) {

        m_socket->sendMsg(jsonstring);
       // QMessageBox::information(&window, "注册成功", "用户名: " + username + "\n密码: " + password);缺乏回馈
      // delete window;
    });
...
void LoginDialog::onLoginBtn()
{
    ...
    if (NULL != m_socket) {
        ...
        message_value["Password"]= ::curUserInfo.passwd;
